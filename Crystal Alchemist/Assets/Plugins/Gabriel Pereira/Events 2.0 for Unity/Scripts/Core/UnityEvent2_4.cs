...
namespace UnityEngine.Events
{
	...
	/// </summary>
	[Serializable]
	public abstract class UnityEvent2<T1, T2, T3, T4> : UnityEventBase2
	{
		...
		{
			return GetValidMethodInfo(targetObj, name, new Type[4] { typeof(T1), typeof(T2), typeof(T3), typeof(T4) });
		}

		internal override BaseInvokableCall2 GetDelegate(object target, MethodInfo theFunction)
		{
			return new InvokableCall2<T1, T2, T3, T4>(target, theFunction);
		}

		private static BaseInvokableCall2 GetDelegate(UnityAction<T1, T2, T3, T4> action)
		{
			return new InvokableCall2<T1, T2, T3, T4>(action);
		}
		...
		public void Invoke(T1 arg1, T2 arg2, T3 arg3, T4 arg4)
		{
			...
			{
				var curCall = calls[i] as InvokableCall2<T1, T2, T3, T4>;
				if (curCall != null)
		...
		internal void AddPersistentListener(UnityAction<T1, T2, T3, T4> call)
		{
			AddPersistentListener(call, UnityEventCallState.RuntimeOnly);
		}

		internal void AddPersistentListener(UnityAction<T1, T2, T3, T4> call, UnityEventCallState callState)
		{
		...
		internal void RegisterPersistentListener(int index, UnityAction<T1, T2, T3, T4> call)
		{
			if (call == null)
