...
namespace UnityEngine.Events
{
	...
	/// </summary>
	[Serializable]
	public abstract class UnityEvent2<T1, T2> : UnityEventBase2
	{
		...
		{
			return GetValidMethodInfo(targetObj, name, new Type[2] { typeof(T1), typeof(T2) });
		}

		internal override BaseInvokableCall2 GetDelegate(object target, MethodInfo theFunction)
		{
			return new InvokableCall2<T1, T2>(target, theFunction);
		}

		private static BaseInvokableCall2 GetDelegate(UnityAction<T1, T2> action)
		{
			return new InvokableCall2<T1, T2>(action);
		}
		...
		public void Invoke(T1 arg1, T2 arg2)
		{
			...
			{
				var curCall = calls[i] as InvokableCall2<T1, T2>;
				if (curCall != null)
		...
		internal void AddPersistentListener(UnityAction<T1, T2> call)
		{
			AddPersistentListener(call, UnityEventCallState.RuntimeOnly);
		}

		internal void AddPersistentListener(UnityAction<T1, T2> call, UnityEventCallState callState)
		{
		...
		internal void RegisterPersistentListener(int index, UnityAction<T1, T2> call)
		{
			if (call == null)
